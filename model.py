...
class DemoireBlock(nn.Module):
    def __init__(self, conv, dim, kernel_size):
        super(DemoireBlock, self).__init__()
...
class Demoire(nn.Module):
    def __init__(self):
        ...
        res2net101.load_state_dict(model_zoo.load_url(model_urls['res2net101_v1b_26w_4s'],map_location=device))
        ...
        self.mid_conv = DemoireBlock(default_conv, 1024, 3)

        self.up_block1 = nn.PixelShuffle(2)
        self.attention1 = DemoireBlock(default_conv, 256, 3)
        self.attention2 = DemoireBlock(default_conv, 192, 3)  
        self.enhancer = Enhancer(28, 28)
        self.tail = nn.Sequential(nn.ReflectionPad2d(3), nn.Conv2d(28, 3, kernel_size=7, padding=0),nn.ReLU())
